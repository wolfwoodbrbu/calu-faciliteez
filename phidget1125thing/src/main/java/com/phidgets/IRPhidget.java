...
/**
 * This class represents a Phidget IR. All methods to send and receive IR data
 * are implemented in this class.
 * <p>
 * The Phidget IR Receiver-Transmitter can send and receive Consumer-IR signals.
 * Ability to learn and re-transmit codes, as well as low-level access to raw
 * data, is provided.
 * 
 * @author Phidgets Inc.
 */
public final class IRPhidget extends Phidget {
	 ...
	 * This can be considered a period of no IR activity. This is used with
	 * {@link #readRaw readRaw}
	 */
	 ...
	 */
	public native void transmit(IRCode code, IRCodeInfo codeInfo)
			throws PhidgetException;

	/**
	 * Transmits a repeat. This needs to be called within the gap time of a
	 * transmit to be meaningful.
	 * 
	 * @throws PhidgetException
	 *             If this Phidget is not opened and attached. See
	 *             {@link com.phidgets.Phidget#open(int) open} for information
	 ...
	 * @param offset
	 *            offset in the data array to start transmitting
	 * @param count
	 ...
	 *             on determining if a device is attached.
	 */
	public native void transmitRaw(int[] data, int offset, int count, int gap,
			int carrierFrequency, int dutyCycle) throws PhidgetException;
	 ...
	 *             on determining if a device is attached.
	 */
	public final void transmitRaw(int[] data, int offset, int count)
			throws PhidgetException {
	 ...
	 *             on determining if a device is attached.
	 */
	public final void transmitRaw(int[] data, int offset, int count, int gap)
			throws PhidgetException {
		transmitRaw(data, offset, count, gap, 0, 0);
	}

	/**
	 * Reads raw data. Use {@link #RAWDATA_LONGSPACE RAWDATA_LONGSPACE} to
	 * detect gaps in IR data.
	 * 
	 * @param buffer
	 *            array into which data will be read.
	 * @param offset
	 *            offset in data to start writing
	 * @param count
	 ...
	 *             on determining if a device is attached.
	 */
	public native int readRaw(int[] buffer, int offset, int count)
			throws PhidgetException;
	...
	/**
	 * Returns the last received code. This is updated right after the code
	 * event returns.
	 * 
	 * @return last code
	 * @throws PhidgetException
	...
	/**
	 * Returns the last learned code. This is updated right after the learn
	 * event returns.
	 * 
	...
	/**
	 * Adds a code listener. The code handler is a method that will be called
	 * when a new code is seen by the reader. The event is fired on each code,
	 ...
	 *            {@link com.phidgets.event.CodeListener CodeListener} interface
	 */
	...
	/**
	 * Adds a code learn listener. The learn handler is a method that will be
	 * called when a new code is learned by the reader. This requires that the
	 * code be repeated several times.
	 * <p>
	 ...
	 * @param l
	 *            An implemetation of the
	 *            {@link com.phidgets.event.LearnListener LearnListener}
	 *            interface
	 */
	...
	/**
	 * Adds a rawData listener. The rawData handler is a method that will be
	 * called when a raw IR data is received.
	 * <p>
	 * There is no limit on the number of rawData handlers that can be
	 * registered for a particular Phidget.
	 * 
	 * @param l
	 *            An implemetation of the
	 *            {@link com.phidgets.event.RawDataListener RawDataListener}
	 *            interface
	 */
