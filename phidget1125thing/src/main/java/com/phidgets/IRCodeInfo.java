...
public final class IRCodeInfo {
	...
	private int encoding = ENCODING_UNKNOWN, length = LENGTH_UNKNOWN,
			bitCount = 0, gap = 0, trail = 0, minRepeat = 1,
			carrierFrequency = 38000, dutyCycle = 33;
	private int[] header = null, one = new int[] { 0, 0 }, zero = new int[] {
			0, 0 }, repeat = null;
	 ...
	 * 
	 * @param encoding
	 *            the encoding - one of {@link #ENCODING_SPACE ENCODING_SPACE},
	 *            {@link #ENCODING_PULSE ENCODING_PULSE},
	 *            {@link #ENCODING_BIPHASE ENCODING_BIPHASE},
	 *            {@link #ENCODING_RC5 ENCODING_RC5} or {@link #ENCODING_RC6
	 *            ENCODING_RC6}
	 * @param bitCount
	 *            the bit count
	 */
	 ...
	 *            the bit count
	 * @param header
	 *            the header (pulse,space)
	 * @param zero
	 *            the zero (pulse, space)
	 * @param one
	 *            the one (pulse, space)
	 * @param trail
	 *            the trailing pulse
	 ...
	 *            the trailing gap
	 */
	public IRCodeInfo(int encoding, int bitCount, int[] header, int[] zero,
			int[] one, int trail, int gap) throws PhidgetException {
		this.encoding = encoding;
		this.bitCount = bitCount;
		...
		if (header != null) {
			if (header.length != 2)
				throw new PhidgetException(
						PhidgetException.EPHIDGET_INVALIDARG,
						"header must have 2 elements");
			this.header = new int[] { header[0], header[1] };
		}

		if (zero == null || zero.length != 2)
			throw new PhidgetException(PhidgetException.EPHIDGET_INVALIDARG,
					"zero must have 2 elements");
		this.zero = new int[] { zero[0], zero[1] };

		if (one == null || one.length != 2)
			throw new PhidgetException(PhidgetException.EPHIDGET_INVALIDARG,
					"one must have 2 elements");
	 ...
	 *            the trailing gap
	 * @param repeat
	 *            the special repeat code
	 */
	public IRCodeInfo(int encoding, int bitCount, int[] header, int[] zero,
			int[] one, int trail, int gap, int[] repeat)
	 ...
	 *            the special repeat code
	 * @param minRepeat
	 *            the minimum number of times to repeat the code
	 * @param toggleMask
	 *            the bits to toggle when minRepeat is > 1
	 * @param length
	 *            the length style - one of {@link #LENGTH_CONSTANT
	 *            LENGTH_CONSTANT} or {@link #LENGTH_VARIABLE LENGTH_VARIABLE}
	 ...
	 */
	public IRCodeInfo(int encoding, int bitCount, int[] header, int[] zero,
			int[] one, int trail, int gap, int[] repeat, int minRepeat,
			IRCode toggleMask, int length, int carrierFrequency, int dutyCycle)
			throws PhidgetException {
		this.encoding = encoding;
		this.bitCount = bitCount;
		...
		if (toggleMask != null)
			toggleMask = new IRCode(toggleMask.getData(),
					toggleMask.getBitCount());
